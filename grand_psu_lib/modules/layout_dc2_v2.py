...
class Layout_dc2:
    ...
    def get_global_lists(self):
        ...
        event_res_tab = np.zeros((len(self.event_list), 70)) - 1
        event_chi2tab = np.zeros((len(self.event_list), 200)) - 1


        for k, ev_id in enumerate(self.event_list[0:self.n_events]):
            print('k=', k, ev_id)
            ...
            if n_du > 0:
                ...
                if n_above_threshold >= self.n_trig_thres:
                    ...
                    if do_swf:
                        ...
                        swf_fit_v4, chi2, ndof, valid = swf.get_SWF_fit_v3(x_ants, t_ants, initial_guess_v2, sigma_t=self.sigma_timing, ncall=self.ncall, varying_nr=True)
                        swf_fit_v5, chi2, ndof, valid = swf.get_SWF_fit_v4(x_ants, t_ants, initial_guess_v2, sigma_t=self.sigma_timing, ncall=self.ncall, varying_nr=False)
                        ...
                        event_res_tab[k, 35] = swf_fit_v5[0]
                        event_res_tab[k, 36] = swf_fit_v5[1]
                        event_res_tab[k, 37] = swf_fit_v5[2]
    ...
    def make_shortcuts(self):
        ...
        self.ang_err = np.arccos(
            np.cos(self.theta_pred * D2R) * np.cos(self.theta_gt*D2R) + np.cos(self.res_phi*D2R) * np.sin(self.theta_pred*D2R) * np.sin(self.theta_gt*D2R)
        )
    ...
    def plot_event(self, id):
        ...
        plt.arrow(-self.shc_y[id]/1000, self.shc_x[id]/1000, 1 * -np.sin(az_pwf+np.pi), 1*np.cos(az_pwf+np.pi), zorder=10, width=.1, color='b' )
        plt.arrow(-self.shc_y[id]/1000, self.shc_x[id]/1000, 5 * -np.sin(az_pwf+np.pi), 5*np.cos(az_pwf+np.pi), zorder=10, width=.1, color='b')
        plt.arrow(-self.shc_y[id]/1000, self.shc_x[id]/1000, 8 * -np.sin(az_pwf+np.pi), 8*np.cos(az_pwf+np.pi), zorder=10, width=.1, color='b')
    ...
    def compute_swf_long_lat_errors(self, Xrec):
        ktab = thetaphi_to_k(self.event_res_tab[:, 2]*D2R, self.event_res_tab[:, 3]*D2R)
        n_tot = self.event_res_tab.shape[0]
        Xmax = self.event_res_tab[:, 16:19]

        _DeltaLong = [np.dot((Xrec-Xmax)[i], (ktab)[:, i]) for i in range(n_tot)]
        _DeltaLat = [np.linalg.norm(np.cross((Xrec-Xmax)[i], (ktab)[:, i])) for i in range(n_tot)]

...
title_str = 'gaa_20240405_074134_RUN003002_CD_phys.root, event {}'.format(iev)
...
for i in range(len(tadc_ev.du_id)):
    df = 0.5
    tr_filtered = filt.filter2(trace_[i, 0,], f1, f2, f3, f4, df, 1)
    axs[i].plot(tr_filtered, label='DU{}'.format(tadc_ev.du_id[i]))
...
for i in range(len(tadc_ev.du_id)):
    df = 0.5
    tr_filtered = filt.filter2(trace_[i, 0,], f1, f2, f3, f4, df, 1)
    axs[i].plot(timex[i], tr_filtered, label='DU{}'.format(tadc_ev.du_id[i]))
